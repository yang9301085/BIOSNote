...
> SIO的看门狗有两种复位机制：
> PowerGood复位
> KRST（Keyboard reset）复位
> ![[Pasted image 20250512135115.png]]
以ITE8659为例：

![[Pasted image 20250512134543.png]]
![[Pasted image 20250512135211.png]]
8659的spec里面只有一个KRST所以只搞这一个就行
`0x72`的bit5可以切换成固定的reset时间或者是自定义reset时间
bit6置为1开启KRST
bit7选择秒或者分计时
`0x73,0x74`这两个地方设置时间
![[Pasted image 20250512135633.png]]
```C
        IoWrite8(SIO_CONFIG_INDEX, IT8659_LDN_SEL_REGISTER);
        IoWrite8(SIO_CONFIG_DATA, IT8659_LDN_GPIO);//Select LGD GPIO
        IoWrite8(SIO_CONFIG_INDEX, 0x72);//WDT Configuration Register 0x72
        WdtSelect=IoRead8(SIO_CONFIG_DATA);
        if (SetupData.SIOWDTTimeoutEnable)
        {
            WdtSelect &= ~BIT5;//BIT5 is WDT timeout value extra select,set to 0 to determined by the WDT timeout value select 1 (bit 7)
            WdtSelect|=BIT6;//BIT6 is WDT Output through KRST (pulse) Enable,set to 1 enable

            //Bit7 is WDT timeout value select 1,set to 1 to switch to second,0 to switch to minute-Start
            WdtSelect |= ((SetupData.WDTTimeoutSelect & 0x01) << 7);
            //Bit7 is WDT timeout value select 1,set to 1 to switch to second,0 to switch to minute-End

            //Config WDT timeout value Register 0x73(LMB),0x74(MSB)-Start
            IoWrite8(SIO_CONFIG_INDEX, 0x73);
            IoWrite8(SIO_CONFIG_DATA,  (UINT8)(SetupData.WDTTimeoutValue & 0xFF));
            IoWrite8(SIO_CONFIG_INDEX, 0x74);
            IoWrite8(SIO_CONFIG_DATA,  (UINT8)(SetupData.WDTTimeoutValue >> 8));
            //Config WDT timeout value Register 0x73(LMB),0x74(MSB)-End
        }else{
            WdtSelect |=BIT5;//BIT5 is WDT timeout value extra select,set to 0 to determined by the WDT timeout value select 1 (bit 7)
            WdtSelect &=~BIT6;//BIT6 is WDT Output through KRST (pulse) Enable,set to 0 disable
        }
        IoWrite8(SIO_CONFIG_INDEX, 0x72);//WDT Configuration Register 0x72
        IoWrite8(SIO_CONFIG_DATA, WdtSelect);
```
