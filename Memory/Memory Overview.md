...
      早期计算机比现在更为简单。系统的各种组件(子系统)例如CPU，内存，大容量存储器和网口，由于被共同开发因而有非常均衡的表现。例如，内存和网口并不比CPU在提供数据的时候更（特别的）快。

      曾经计算机稳定的基本结构悄然改变，硬件开发人员开始致力于优化单个子系统。于是电脑一些组件的性能大大的落后因而成为了瓶颈。由于成本的原因，大容量存储器和内存子系统相对于其他组件来说更难于提高性能。

      大容量存储的性能问题往往靠软件来改善: 操作系统将常用(且最有可能被用)的数据放在主存中，因为主存的读写速度要比外部存储器(例如硬盘)快上几个数量级。或者将缓存加入存储设备中(例如硬盘中的高速缓存)，这样就可以在不修改操作系统的前提下提升性能。{然而，为了在使用缓存时保证数据的完整性，仍然要作出一些修改。}这些内容不在本文的谈论范围之内，就不作赘述了。
...
      鉴于目前专业硬件正在逐渐淡出，理解商用硬件的现状变得十分重要。现如今，人们更多的采用水平扩展，也就是说，用大量小型、互联的商用计算机代替巨大、超快(但超贵)的系统。原因在于，快速而廉价的网络硬件已经崛起。那些大型的专用系统仍然有一席之地，但已被商用硬件后来居上。2007年，红帽子公司(Red Hat)认为，未来构成数据中心的“积木”将会是拥有最多4个插槽的计算机，每个插槽插入一个四核CPU，这些CPU都是超线程的。{超线程使单个处理器核心能同时处理两个以上的任务，只需加入一点点额外硬件}。也就是说，这些数据中心中的标准系统拥有最多64(??)个虚拟处理器。当然可以支持更大的系统，但人们认为4插槽、4核CPU是最佳配置，绝大多数的优化都针对这样的配置。
...
![[Pasted image 20240220093457.png]]
> 图2.1 北桥和南桥组成的结构

      CPU通过一条通用总线(前端总线，FSB)连接到北桥[现在的cpu都是集成北桥]。北桥主要包括内存控制器和其它一些组件，内存控制器决定了RAM芯片的类型。不同的类型，包括DRAM、Rambus和SDRAM等等，要求不同的内存控制器。
...
1. 从某个CPU到另一个CPU的数据需要走它与北桥通信的同一条总线。
2. 与RAM的通信需要经过北桥
3. RAM只有一个端口。{本文不会介绍多端口RAM，因为商用硬件不采用这种内存，至少程序员无法访问到。这种内存一般在路由器等专用硬件中采用。}
4. CPU与南桥设备间的通信需要经过北桥。
...
![[Pasted image 20240220093602.png]]
...
![[Pasted image 20240220093642.png]]
图2.3 集成的内存控制器

      通过采用这样的架构，系统里有几个处理器，就可以有几个内存bank。比如，在4 CPU的计算机上，不需要一个拥有巨大带宽的复杂北桥，就可以实现4倍的内存带宽。另外，将内存控制器集成到CPU内部还有其它一些优点，这里就不赘述了。
...
> ![[Pasted image 20240220093650.png]]
> 图2.6 6-T静态RAM
...
1. 一个单元需要6个晶体管。也有采用4个晶体管的SRAM，但有缺陷。
2. 维持状态需要恒定的电源。
3. 升起WL后立即可以读取状态。信号与其它晶体管控制的信号一样，是直角的(快速在两个状态间变化)。
4. 状态稳定，不需要刷新循环。
...
> ![[Pasted image 20240220093702.png]]
> 
> 图2.5 1-T动态RAM
...
      一个程序选择了一个内存位置使用到了一个虚拟地址。处理器转换这个地址到物理地址，最后内存控制选择将RAM芯片匹配了那个地址。RAM芯片再去选择单个内存单元，部分的物理地址以许多地址行的形式被传递。

     单独地去处理来自于内存控制器的内存位置将完全不切实际：4G的RAM将需要 232 地址行。DRAM芯片地址传递的这种方式首先信号分离器解析。一个信号分离器的N多地址行将有2_N_ 输出行。这些输出行能被使用到选择内存单元。使用这个直接方法对于小容量芯片不再是个大问题。

      但对于大的内存这种方法不再适合。一个1G的芯片容量（我反感那些SI前缀，对于我一个_giga-bit将总是_230_ 而不是_109__字节）将需要30地址行和230 选项行。随着地址行的增加，信号分离器的复杂度将会以指数复杂度递增。更加困难的是，在地址行传输30路同步脉冲，要比传输15路同步脉冲复杂很多。因为同步脉冲路数越多，就越难实现长度和时间上的精确同步｛注意现在DDR类型，比如DDR3都有自动跳转时序功能，以便匹配时序和长度同步}

![[Pasted image 20240220093749.png]]

      图2.7展示了一个DRAM芯片的细节组成，可以看到DRAM通过行和列的方式组织起来。我们可以对一行的DRAM进行集中访问，但这需要一个很大的信号分离器。通过阵列方法设计能被一个信号分离器和一个半的multiplexer获得{多路复用器（multiplexer）和信号分离器是一样的，这的multiplexer需要以信号分离器身份工作当写数据时候。那么从现在开始我们开始讨论其区别.}这在所有方面会是一个大的存储。例如地址linesa0和a1通过行地址选择路由器来选择整个行的芯片的地址列，当读的时候，所有的芯片目录能使其纵列选择路由器可用，依据地址linesa2和a3一个纵列的目录用于数据DRAM芯片的接口类型。这发生了许多次在许多DRAM芯片产生一个总记录数的字节匹配给一个宽范围的数据总线。

      对于写操作，内存单元的数据新值被放到了数据总线，当使用RAS和CAS方式选中内存单元时，数据是存放在内存单元内的。这是一个相当直观的设计，在现实中很显然会复杂得多，对于读，需要规范从发出信号到数据在数据总线上变得可读的时延。电容不会像前面章节里面描述的那样立刻自动放电，从内存单元发出的信号是如此这微弱以至于它需要被放大。对于写，必须规范从数据RAS和CAS操作完成后到数据成功的被写入内存单元的时延（当然，电容不会立刻自动充电和放电）。这些时间常量对于DRAM芯片的性能是至关重要的，我们将在下章讨论它。
...
2. 存储单元需要单独选择来使用
3. 地址线数目直接负责存储控制器，主板，DRAM模块和DRAM芯片的成本
4. 在读或写操作结果之前需要占用一段时间是可行的
...
>![[Pasted image 20240220093837.png]]
> 图2.8: SDRAM读访问的时序
...
>![[Pasted image 20240220093854.png]]
> 图2.9: SDRAM的预充电与激活
...
> [](http://images.cnitblog.com/blog/361409/201304/13091201-f36090192a9e4cd68ad59b0d5deb3967.png)[![06104433_JWAj](http://images.cnitblog.com/blog/361409/201304/13091202-cd6a843b28964cb1b05e2a7c76d7034a.png "06104433_JWAj")](http://images.cnitblog.com/blog/361409/201304/13091202-684854e13fd24fe4800ddc53c5460ef3.png)   
> 图2.10: SDR SDRAM的操作

      在图2.10中，DRAM单元阵列能以等同于内存总线的速率输出内容。假设DRAM单元阵列工作在100MHz上，那么总线的数据传输率可以达到100Mb/s。所有组件的频率_f_保持相同。由于提高频率会导致耗电量增加，所以提高吞吐量需要付出很高的的代价。如果是很大规模的内存阵列，代价会非常巨大。{_功率 = 动态电容 x 电压__2_ _x 频率_}。而且，提高频率还需要在保持系统稳定的情况下提高电压，这更是一个问题。因此，就有了DDR SDRAM(现在叫DDR1)，它可以在不提高频率的前提下提高吞吐量。

> [](http://images.cnitblog.com/blog/361409/201304/13091203-8517c1d416814c71b13ba25a6b1a2c5b.png)[![06104433_A6Jf](http://images.cnitblog.com/blog/361409/201304/13091204-1bbdc4814ca445a89ef322687c082dea.png "06104433_A6Jf")](http://images.cnitblog.com/blog/361409/201304/13091204-9a889470a3174ff48814216e83892de9.png)   
> 图2.11 DDR1 SDRAM的操作
...
>   ![[Pasted image 20240220093912.png]]
> 图2.12: DDR2 SDRAM的操作
...
[![image](http://images.cnitblog.com/blog/361409/201304/13091210-5630b93478db42a99ca95bec1ca9cebe.png "image")](http://images.cnitblog.com/blog/361409/201304/13091206-d0739761d152468bbb21ad3608376f04.png)
...
>  ![[Pasted image 20240220093925.png]]
> **图2.14: DDR3 SDRAM的操作**
...
[](http://images.cnitblog.com/blog/361409/201304/13091212-7249bf66d1d94f65a677692753bc0859.png)[![image](http://images.cnitblog.com/blog/361409/201304/13091217-8187bae33c0f4458aad27cce129a0780.png "image")](http://images.cnitblog.com/blog/361409/201304/13091213-7522c867c38549fabe5f181f52c4459d.png)

      DDR有一个共同的缺点，由于有较高的频率，很难创建并行的数据总线。DDR2有240个脚，所有连线都需要合理排布，保证它们有基本相同的长度。更复杂的是，如果有多个DDR模块通过菊花链(daisy-chained)的方式接在同一个总线上，信号会被干扰，模块越多，干扰越重。DDR2规范只允许同一总线(即通道)接入两个模块，DDR规范只允许高频率下连接一个模块。由于DDR2的引脚多达240只，单个北桥不大可能连接2个以上的通道。要么改用外部内存控制(如图2.2)，但那很贵。

      根据以上的推理，一个商用主板最多只能容纳4个DDR2或DDR3模块。这样一来，系统的内存容量就被大大地限制了。即使是古老的32位IA-32处理器也能支持64GB内存，而且就算仅仅是家用，内存需求也在不断看涨，因此内存技术方面有必要做些什么了。
...
[![image](http://images.cnitblog.com/blog/361409/201304/13091219-9fad31f05bb84a948bd7b0c29a0d01ee.png "image")](http://images.cnitblog.com/blog/361409/201304/13091218-6c0fd827f06a46ecaacb107c9c3eec28.png)
